...
agent_instructions_template <- function() {
    c("# Agent Instructions", "",
        ...
        "file when no entries remain, then commit.", "",
        "## What lives here", "", "```",
...
schema_template <- function() {
    c(
        ...
        "## .pensarignore",
        "",
        "Optional file at vault root. One glob per line. `#` comments and blank",
        "lines are ignored. No negation patterns.",
        "",
        "Scope: filters the **synthesis backlog** only (`pensar lint` raw orphans",
        "and tag clusters). Does not affect ingest, indexing, broken-link checks,",
        "or the registry.",
        "",
        "Use for content that should never appear in the backlog (e.g. personal",
        "todo lists, scratch notes). Do not use for \"pending\" content that has",
        "its own processing path.",
        "",
        "Glob semantics: trailing `*` is a prefix match, so `raw/scratch/*`",
        "also recurses into subdirectories (matches `raw/scratch/sub/foo.md`).",
        "This is friendlier than gitignore's child-only semantics, but means",
        "`raw/articles/foo*` will match `raw/articles/foo/bar.md` too.",
        "",
        "## Drill-Down Workflow",
        "",
