...
#' }
#' @export
ingest_url <- function(url, vault = default_vault(),
                       type = "articles", title = NULL,
                       tags = NULL) {
    ...
    if (is.null(title) && grepl("html", ctype, ignore.case = TRUE)) {
        title <- extract_html_title(body) %||% NULL
    }
    if (is.null(title)) {
        title <- url
    }

    fp <- ingest(body, type = type, source = url, title = title,
